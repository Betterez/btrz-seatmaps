...
var SeatmapSocket = /*#__PURE__*/function () {
  ...
  _createClass(SeatmapSocket, null, [{
    ...
    key: "pushEvent",
    value: function pushEvent(name, seat) {
      var payload = {
        seat: {
...
var SeatmapSection = /*#__PURE__*/function () {
  ...
  _createClass(SeatmapSection, [{
    ...
    value: function onSeatClicked(evt, e, seat) {
      if (e.dataset.status === "available" && this.socketEvents.callbacks.seatSelected(seat, {
        tripId: this.socketEvents.tripId,
        scheduleId: this.socketEvents.scheduleId
      })) {
        ...
        this.socketEvents.callbacks.seatUnselected(seat, {
          tripId: this.socketEvents.tripId,
          scheduleId: this.socketEvents.scheduleId
        });
      }
    ...
    value: function focusOnNextSelected() {
      ...
      for (var i = index; i < this.seats.length + 1; i++) {
        if (_classPrivateMethodGet(this, _setFocusOnSeat, _setFocusOnSeat2).call(this, i, true)) {
          break;
        }
      }
    }
...
function _updateFacilities2() {
  ...
  this.facilities.forEach(function (ele) {
    if (ele.type === "driver") {
      ...
      ele.width = _this26.availableCols;
      ele.row = ele.position.key === _class2.FACILITY_POSITION.Top ? 1 : _this26.availableRows;
      filteredElements.push(ele);
    } else if (ele.type === "door") {
      ele.col = ele.alignment.key === _class2.FACILITY_ALIGNMENT.Left ? 1 : _this26.availableCols;
      filteredElements.push(ele);
...
function _buildCorridor2() {
  ...
  rows.forEach(function (row, rIndex) {
    ...
    _this27.corridor.push({
      ...
      alternativeLabel: _this27.rowLabelType === _class2.LABEL_TYPES.Number ? rIndex + 1 : String.fromCharCode(96 + rIndex + 1).toUpperCase()
    });
    if (showLabel || !_this27.rowsEnumNoGaps) {
      nextRowValue = _this27.rowLabelType === _class2.LABEL_TYPES.Number ? parseInt(nextRowValue, 10) + 1 : String.fromCharCode(nextRowValue.charCodeAt(0) + 1);
    }
  });
}
...
function _enumerateSeats2(section) {
  ...
  rows.forEach(function (row, rIndex) {
    _this29.seats.filter(function (s) {
      ...
      return _this29.enumDir === _class2.ENUMERATION_DIRECTION.Left ? a.col - b.col : b.col - a.col;
    }).forEach(function (s, index) {
      if (_this29.enumType === _class2.ENUMERATION_TYPES.PerRow) {
        if (rowNumber !== s.row) {
      ...
      s.index = seatIndex;
      seatLabel = _this29.seatLabelType === _class2.LABEL_TYPES.Number ? parseInt(seatLabel, 10) + 1 : String.fromCharCode(seatLabel.charCodeAt(0) + 1);
      seatIndex++;
    });
  });
}
...
function _setFocusOnSeat2(index) {
  var excludeSelected = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  var focus = document.querySelector("[data-focus]");
  var selector = excludeSelected ? "[data-keynav='true'][data-index='".concat(index, "']:not([data-selected=true])") : "[data-keynav='true'][data-index='".concat(index, "']");
