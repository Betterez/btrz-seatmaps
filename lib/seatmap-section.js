    ...
    value: function draw() {
      ...
      if (container) {
        ...
        [].concat(_toConsumableArray(this.corridor), _toConsumableArray(this.facilities), _toConsumableArray(this.seats)).forEach(function (elem) {
          ...
          var label = _assertClassBrand(_SeatmapSection_brand, _this25, _createHTMLElement).call(_this25, "div", "", elem.label || "");
          var e = _assertClassBrand(_SeatmapSection_brand, _this25, _createHTMLElement).call(_this25, "div", classes, label.outerHTML);
...
function _updateFacilities() {
  ...
  this.facilities.forEach(function (ele) {
    if (ele.type === "driver") {
      // eslint-disable-next-line no-param-reassign
      ele.width = _this29.availableCols;
      filteredElements.push(ele);
    } else if (ele.type === "accordion") {
      ele.width = _this29.availableCols;
      ele.row = ele.position.key === _SeatmapSection.FACILITY_POSITION.Top ? 1 : _this29.availableRows;
      filteredElements.push(ele);
    } else if (ele.type === "door") {
      ele.col = ele.alignment.key === _SeatmapSection.FACILITY_ALIGNMENT.Left ? 1 : _this29.availableCols;
      filteredElements.push(ele);
    } else if (_this29.availableCols + 1 >= ele.col + ele.width) {
      filteredElements.push(ele);
    }
  });
  // eslint-disable-next-line no-param-reassign
...
function _buildCorridor() {
  ...
  rows.forEach(function (row, rIndex) {
    var showLabel = _this30.showRowLabels && (!rowsWithoutLabels.length || !rowsWithoutLabels.includes(rIndex + 1));
    _this30.corridor.push({
      row: rIndex + startingRowPosition,
      col: _this30.seatsPerRowLeft + 1,
      height: 1,
      width: 1,
      type: "corridor",
      classes: _this30.classes.corridor,
      label: showLabel ? nextRowValue : undefined,
      alternativeLabel: _this30.rowLabelType === _SeatmapSection.LABEL_TYPES.Number ? rIndex + 1 : String.fromCharCode(96 + rIndex + 1).toUpperCase()
    });
    if (showLabel || !_this30.rowsEnumNoGaps) {
      nextRowValue = _this30.rowLabelType === _SeatmapSection.LABEL_TYPES.Number ? parseInt(nextRowValue, 10) + 1 : String.fromCharCode(nextRowValue.charCodeAt(0) + 1);
    }
  });
}
function _buildSeats() {
  ...
  rows.forEach(function (row, rIndex) {
    cols.forEach(function (col, cIndex) {
      var overlaps = _assertClassBrand(_SeatmapSection_brand, _this31, _overlapsWithFixedElement).call(_this31, [].concat(_toConsumableArray(_this31.facilities), _toConsumableArray(_this31.corridor)), rIndex + 1, cIndex + 1);
      ...
      if (!hideSeat) {
        ...
        var customSeat = _this31.customSeats.find(function (st) {
          return st.col === colNumber && st.row === rowNumber;
        });
        var rowLabel = _assertClassBrand(_SeatmapSection_brand, _this31, _getSeatRowLabel).call(_this31, rowNumber);
        var label = customSeat ? customSeat.label : "";
        var status = customSeat ? customSeat.status : "available";
        if (status === "reserved" && !_this31.isEditing) {
          status = _this31.isBackOffice ? "reserved" : "blocked";
        }
        var allowKeyNav = (_this31.allowKeyNavStatusList.includes(status) || status === "reserved") && !isOverlapped;
        var seat = {
          sectionId: _this31.sectionId,
          containerId: _this31.containerId,
          type: "seat",
          classes: _this31.classes.seat,
          row: rowNumber,
          col: colNumber,
          height: 1,
          width: 1,
        ...
        seat.seatId = _this31.getSeatId(seat);
        if (_this31.allowSeatOrientation) {
...
function _enumerateSeats(section) {
  ...
  var seatIndex = 1;
  rows.forEach(function (row, rIndex) {
    _this32.seats.filter(function (s) {
      return s.row === rIndex + 1;
    }).sort(function (a, b) {
      return _this32.enumDir === _SeatmapSection.ENUMERATION_DIRECTION.Left ? a.col - b.col : b.col - a.col;
    }).forEach(function (s, index) {
      if (_this32.enumType === _SeatmapSection.ENUMERATION_TYPES.PerRow) {
        if (rowNumber !== s.row) {
          seatLabel = _this32.startingSeatLabel;
          rowNumber++;
        }
      }
      s.label = s.label || seatLabel;
      s.index = seatIndex;
      seatLabel = _this32.seatLabelType === _SeatmapSection.LABEL_TYPES.Number ? parseInt(seatLabel, 10) + 1 : String.fromCharCode(seatLabel.charCodeAt(0) + 1);
      seatIndex++;
    });
  });
}
...
function _manageKeyboardNavigation(evt) {
  ...
  if (evt.keyCode >= 48 && evt.keyCode <= 57 || evt.keyCode >= 96 && evt.keyCode <= 105) {
    if (this.numPad === "") {
      setTimeout(function () {
        _assertClassBrand(_SeatmapSection_brand, _this33, _setFocusOnSeatByLabel).call(_this33, _this33.numPad);
        _this33.numPad = "";
        _assertClassBrand(_SeatmapSection_brand, _this33, _showNumPad).call(_this33, _this33.numPad);
      }, "500");
    }
    this.numPad += evt.key;
  ...
  return _createClass(SeatmapIframe, [{
    ...
    value: function seatmapJoin() {
      ...
      seats.forEach(function (s) {
        var parsed = s.sectionId && s.containerId ? s : _this27.parseSeat(s.seat_id);
    ...
    value: function expiredSeats() {
      ...
      seats.forEach(function (seat) {
        seat.seat_id = _this28.getSeatId(seat);
        SeatmapSection.changeSeatDataProp(seat, "selected", "false");
        SeatmapSection.changeSeatStatus(seat, "available");
      });
      try {
