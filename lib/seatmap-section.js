    ...
    return _createClass(_Socket, [{
      ...
      value: function teardown(callback, code, reason) {
        ...
        this.waitForBufferDone(function () {
          if (_this17.conn) {
            if (code) {
              _this17.conn.close(code, reason || "");
            } else {
              _this17.conn.close();
            }
          }
          _this17.waitForSocketClosed(function () {
            if (_this17.conn) {
              _this17.conn.onopen = function () {};
              _this17.conn.onerror = function () {};
              _this17.conn.onmessage = function () {};
              _this17.conn.onclose = function () {};
              _this17.conn = null;
            }
            callback && callback();
          });
      ...
      value: function waitForBufferDone(callback) {
        ...
        setTimeout(function () {
          _this18.waitForBufferDone(callback, tries + 1);
        }, 150 * tries);
      }
    }, {
      ...
      value: function waitForSocketClosed(callback) {
        ...
        setTimeout(function () {
          _this19.waitForSocketClosed(callback, tries + 1);
        }, 150 * tries);
      }
    }, {
      key: "onConnClose",
      ...
      value: function push(data) {
        ...
        if (this.isConnected()) {
          this.encode(data, function (result) {
            return _this20.conn.send(result);
          });
        } else {
          this.sendBuffer.push(function () {
            return _this20.encode(data, function (result) {
              return _this20.conn.send(result);
            });
          });
        }
      ...
      value: function sendHeartbeat() {
        ...
        this.heartbeatTimeoutTimer = setTimeout(function () {
          return _this21.heartbeatTimeout();
        }, this.heartbeatIntervalMs);
      }
    }, {
      key: "flushSendBuffer",
      ...
      value: function onConnMessage(rawMessage) {
        ...
        this.decode(rawMessage.data, function (msg) {
          ...
          if (ref && ref === _this22.pendingHeartbeatRef) {
            _this22.clearHeartbeats();
            _this22.pendingHeartbeatRef = null;
            _this22.heartbeatTimer = setTimeout(function () {
              return _this22.sendHeartbeat();
            }, _this22.heartbeatIntervalMs);
          }
          if (_this22.hasLogger()) _this22.log("receive", "".concat(payload.status || "", " ").concat(topic, " ").concat(event, " ").concat(ref && "(" + ref + ")" || ""), payload);
          for (var i = 0; i < _this22.channels.length; i++) {
            var channel = _this22.channels[i];
            if (!channel.isMember(topic, event, payload, join_ref)) {
              continue;
            }
            channel.trigger(event, payload, ref, join_ref);
          }
          for (var _i = 0; _i < _this22.stateChangeCallbacks.message.length; _i++) {
            var _this22$stateChangeCa = _slicedToArray(_this22.stateChangeCallbacks.message[_i], 2),
              callback = _this22$stateChangeCa[1];
            callback(msg);
          }
        });
      }
  ...
  return _createClass(SeatmapSocket, null, [{
    ...
    value: function listen(settings) {
      ...
      function joinChannel(vChannel) {
        vChannel.join().receive("ok", function () {
          console.log("Join successfully to ".concat(vChannel.topic));
          vChannel.on("seat:selected", function (payload) {
          ...
          vChannel.on("seat:unselected", function (payload) {
          ...
          vChannel.on("sync:join", function (payload) {
            console.log("sync:join", payload);
            if (SeatmapSocket.settings.callbacks.seatmapJoin && payload.seats && payload.seats.length) {
              SeatmapSocket.settings.callbacks.seatmapJoin(payload.seats);
            }
          });
          vChannel.on("sync:seats", function (payload) {
...
var SeatmapSection = /*#__PURE__*/function () {
  function SeatmapSection() {
    ...
    this.allowSeatOrientation = _section.allowSeatOrientation;
  ...
  return _createClass(SeatmapSection, [{
    ...
    value: function draw() {
      var _this23 = this;
      var numericDisplay = _assertClassBrand(_SeatmapSection_brand, this, _createHTMLElement).call(this, "div", "numeric-display, absolute, right-0, left-0, text-center, z-[2], font-weight-bolder, opacity-20, text-color-primary");
      var container = document.getElementById(this.containerId);
      if (container) {
        container.onkeydown = function (evt) {
          _assertClassBrand(_SeatmapSection_brand, _this23, _manageKeyboardNavigation).call(_this23, evt);
        };
        ...
        if (this.sectionName && this.showSectionName) {
          var sectionNameWrapper = _assertClassBrand(_SeatmapSection_brand, this, _createHTMLElement).call(this, "div", "absolute, bottom-0, right-0, left-0, text-center, -mb-200, z-[2]");
          var sectionNameContainer = _assertClassBrand(_SeatmapSection_brand, this, _createHTMLElement).call(this, "div", "bg-bz-info-default, uppercase, text-size-87, text-bz-info-lightest, shadow-elevation-flat, border, border-bz-info-light, inline-block, rounded-pill, px-100, leading-175", this.sectionName);
        ...
        [].concat(_toConsumableArray(this.corridor), _toConsumableArray(this.facilities), _toConsumableArray(this.seats)).forEach(function (elem) {
          var classes = _assertClassBrand(_SeatmapSection_brand, _this23, _getElementClasses).call(_this23, elem);
          var label = _assertClassBrand(_SeatmapSection_brand, _this23, _createHTMLElement).call(_this23, "div", "", elem.label || "");
          var e = _assertClassBrand(_SeatmapSection_brand, _this23, _createHTMLElement).call(_this23, "div", classes, label.outerHTML);
          var dataset = _assertClassBrand(_SeatmapSection_brand, _this23, _getSeatDataset).call(_this23, elem);
          Object.keys(dataset).forEach(function (k) {
            e.dataset[k] = dataset[k];
          });
          if (elem.type === "seat") {
            e.title = SeatmapSection.getSeatTitle(elem, _this23.sectionName, _this23.labels, _this23.seatClasses, _this23.fees);
          }
          _assertClassBrand(_SeatmapSection_brand, _this23, _setElementStyle).call(_this23, e.style, elem);
          _this23.events.forEach(function (evt) {
    ...
    value: function getCapacity() {
      var statusToExclude = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
      return (this.seats || []).filter(function (s) {
        return !s.overlapped && s.status && s.type === "seat" && !statusToExclude.includes(s.status);
      }).length;
    }
  }, {
    key: "selectElement",
    ...
    value: function getSeatTitle(elem, sectionName, labels, seatClasses, fees) {
      ...
      var isEmergency = elem.emergency ? "".concat(labels.emergency) : "";
      var gender = "";
      ...
      return "".concat(section).concat(row).concat(seat).concat(status).concat(seatClassName).concat(feeName).concat(isAccessible).concat(isEmergency).concat(gender).concat(isSuggested);
    }
  }]);
}();
...
function _updateFacilities() {
  ...
  this.facilities.forEach(function (ele) {
    if (ele.type === "driver") {
      // eslint-disable-next-line no-param-reassign
      ele.width = _this27.availableCols;
      filteredElements.push(ele);
    } else if (ele.type === "accordion") {
      ele.width = _this27.availableCols;
      ele.row = ele.position.key === _SeatmapSection.FACILITY_POSITION.Top ? 1 : _this27.availableRows;
      filteredElements.push(ele);
    } else if (ele.type === "door") {
      ele.col = ele.alignment.key === _SeatmapSection.FACILITY_ALIGNMENT.Left ? 1 : _this27.availableCols;
      filteredElements.push(ele);
    } else if (_this27.availableCols + 1 >= ele.col + ele.width) {
      filteredElements.push(ele);
    }
  });
  // eslint-disable-next-line no-param-reassign
...
function _buildCorridor() {
  ...
  rows.forEach(function (row, rIndex) {
    var showLabel = _this28.showRowLabels && (!rowsWithoutLabels.length || !rowsWithoutLabels.includes(rIndex + 1));
    _this28.corridor.push({
      row: rIndex + startingRowPosition,
      col: _this28.seatsPerRowLeft + 1,
      height: 1,
      width: 1,
      type: "corridor",
      classes: _this28.classes.corridor,
      label: showLabel ? nextRowValue : undefined,
      alternativeLabel: _this28.rowLabelType === _SeatmapSection.LABEL_TYPES.Number ? rIndex + 1 : String.fromCharCode(96 + rIndex + 1).toUpperCase()
    });
    if (showLabel || !_this28.rowsEnumNoGaps) {
      nextRowValue = _this28.rowLabelType === _SeatmapSection.LABEL_TYPES.Number ? parseInt(nextRowValue, 10) + 1 : String.fromCharCode(nextRowValue.charCodeAt(0) + 1);
    }
  });
}
function _buildSeats() {
  ...
  rows.forEach(function (row, rIndex) {
    cols.forEach(function (col, cIndex) {
      var overlaps = _assertClassBrand(_SeatmapSection_brand, _this29, _overlapsWithFixedElement).call(_this29, [].concat(_toConsumableArray(_this29.facilities), _toConsumableArray(_this29.corridor)), rIndex + 1, cIndex + 1);
      ...
      if (!hideSeat) {
        ...
        var customSeat = _this29.customSeats.find(function (st) {
          return st.col === colNumber && st.row === rowNumber;
        });
        var rowLabel = _assertClassBrand(_SeatmapSection_brand, _this29, _getSeatRowLabel).call(_this29, rowNumber);
        var label = customSeat ? customSeat.label : "";
        var status = customSeat ? customSeat.status : "available";
        if (status === "reserved" && !_this29.isEditing) {
          status = _this29.isBackOffice ? "reserved" : "blocked";
        }
        var allowKeyNav = (_this29.allowKeyNavStatusList.includes(status) || status === "reserved") && !isOverlapped;
        var seat = {
          sectionId: _this29.sectionId,
          containerId: _this29.containerId,
          type: "seat",
          classes: _this29.classes.seat,
          row: rowNumber,
          col: colNumber,
          height: 1,
          width: 1,
        ...
        seat.seatId = _this29.getSeatId(seat);
        if (_this29.allowSeatOrientation) {
          var orient = customSeat && customSeat.orientation;
          if (orient !== "none") {
...
function _enumerateSeats(section) {
  ...
  var seatIndex = 1;
  rows.forEach(function (row, rIndex) {
    _this30.seats.filter(function (s) {
      return s.row === rIndex + 1;
    }).sort(function (a, b) {
      return _this30.enumDir === _SeatmapSection.ENUMERATION_DIRECTION.Left ? a.col - b.col : b.col - a.col;
    }).forEach(function (s, index) {
      if (_this30.enumType === _SeatmapSection.ENUMERATION_TYPES.PerRow) {
        if (rowNumber !== s.row) {
          seatLabel = _this30.startingSeatLabel;
          rowNumber++;
        }
      }
      s.label = s.label || seatLabel;
      s.index = seatIndex;
      seatLabel = _this30.seatLabelType === _SeatmapSection.LABEL_TYPES.Number ? parseInt(seatLabel, 10) + 1 : String.fromCharCode(seatLabel.charCodeAt(0) + 1);
      seatIndex++;
    });
  });
}
...
function _manageKeyboardNavigation(evt) {
  ...
  if (evt.keyCode >= 48 && evt.keyCode <= 57 || evt.keyCode >= 96 && evt.keyCode <= 105) {
    if (this.numPad === "") {
      setTimeout(function () {
        _assertClassBrand(_SeatmapSection_brand, _this31, _setFocusOnSeatByLabel).call(_this31, _this31.numPad);
        _this31.numPad = "";
        _assertClassBrand(_SeatmapSection_brand, _this31, _showNumPad).call(_this31, _this31.numPad);
      }, "500");
    }
    this.numPad += evt.key;
  ...
  return _createClass(SeatmapIframe, [{
    ...
    value: function drawSeatmapIframe() {
      ...
      this.seatmap.sections.forEach(function (section, index) {
        ...
        var seatmapSection = new SeatmapSection(sectionContainer.id, section, {
          fees: _this24.seatfees,
          seatClasses: _this24.seatClasses,
    ...
    value: function seatmapJoin() {
      ...
      seats.forEach(function (s) {
        var parsed = s.sectionId && s.containerId ? s : _this25.parseSeat(s.seat_id);
        if (!parsed.containerId) {
          parsed.containerId = "seatmapContainer-section-".concat(parsed.sectionId);
        }
    ...
    value: function expiredSeats() {
      ...
      seats.forEach(function (seat) {
        seat.seat_id = _this26.getSeatId(seat);
        SeatmapSection.changeSeatDataProp(seat, "selected", "false");
        SeatmapSection.changeSeatStatus(seat, "available");
      });
      try {
