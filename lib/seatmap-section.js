...
var SeatmapSocket = /*#__PURE__*/function () {
  ...
  _createClass(SeatmapSocket, null, [{
    ...
    value: function listen(settings) {
      ...
      var socket = new Phoenix.Socket(settings.socketUrl, {
        params: {
      ...
      SeatmapSocket.channel = socket.channel("seatmap:".concat(settings.idForLiveSeatmap), {
      ...
      SeatmapSocket.channel.join().receive("ok", function () {
        ...
        SeatmapSocket.channel.on("sync:join", function (payload) {
          ...
          payload.seats.forEach(function (data) {
            var selectedSeat = data.seat || data.seat_id;
        ...
        SeatmapSocket.channel.on("sync:seats", function (payload) {
          ...
          SeatmapSocket.settings.callbacks.seatExpired(payload.expired.map(function (data) {
            return {
              seat_id: "".concat(data.seat.row, "-").concat(data.seat.col, "-").concat(data.seat.label),
              row: data.seat.row,
              col: data.seat.col,
              height: 1,
              width: 1,
              sectionId: _this25.sectionId,
              sectionName: _this25.sectionName
            };
          }), {
    ...
    key: "pushEvent",
    value: function pushEvent(name, seat, section) {
      if (SeatmapSocket.channel) {
        var payload = {
          seat: {
...
var SeatmapSection = /*#__PURE__*/function () {
  ...
  _createClass(SeatmapSection, [{
    ...
    value: function draw() {
      ...
      if (container) {
        container.onkeydown = function (evt) {
          _classPrivateMethodGet(_this26, _manageKeyboardNavigation, _manageKeyboardNavigation2).call(_this26, evt);
        };
        ...
        [].concat(_toConsumableArray(this.corridor), _toConsumableArray(this.facilities), _toConsumableArray(this.seats)).forEach(function (elem) {
          var classes = _classPrivateMethodGet(_this26, _getElementClasses, _getElementClasses2).call(_this26, elem);
          var e = _classPrivateMethodGet(_this26, _createHTMLElement, _createHTMLElement2).call(_this26, "div", classes, elem.label || "");
          var dataset = _classPrivateMethodGet(_this26, _getSeatDataset, _getSeatDataset2).call(_this26, elem);
          Object.keys(dataset).forEach(function (k) {
            e.dataset[k] = dataset[k];
          });
          if (elem.type === "seat") {
            e.title = SeatmapSection.getSeatTitle(elem, _this26.sectionName, _this26.labels, _this26.seatClasses, _this26.fees);
          }
          _classPrivateMethodGet(_this26, _setElementStyle, _setElementStyle2).call(_this26, e.style, elem);
          _this26.events.forEach(function (evt) {
    ...
    value: function focusOnNextSelected() {
      var firstSeatSelected = document.querySelector("[data-keynav='true'][data-selected=true]");
      var index = firstSeatSelected ? firstSeatSelected.dataset.index : 1;
      var container = document.getElementById(this.containerId);
      if (container) {
        container.focus();
      }
    ...
    value: function changeSeatDataProp(elem, prop, value) {
      var selector = "[style*='grid-area: ".concat(elem.row, " / ").concat(elem.col, " / ").concat(parseInt(elem.row, 10) + (elem.height || 1), " / ").concat(parseInt(elem.col, 10) + (elem.width || 1), ";']");
      var element = document.querySelector(selector);
      if (element) {
...
function _updateFacilities2() {
  ...
  this.facilities.forEach(function (ele) {
    if (ele.type === "driver") {
      ...
      ele.width = _this27.availableCols;
      ele.row = ele.position.key === _class2.FACILITY_POSITION.Top ? 1 : _this27.availableRows;
      filteredElements.push(ele);
    } else if (ele.type === "door") {
      ele.col = ele.alignment.key === _class2.FACILITY_ALIGNMENT.Left ? 1 : _this27.availableCols;
      filteredElements.push(ele);
...
function _buildCorridor2() {
  ...
  rows.forEach(function (row, rIndex) {
    ...
    _this28.corridor.push({
      ...
      alternativeLabel: _this28.rowLabelType === _class2.LABEL_TYPES.Number ? rIndex + 1 : String.fromCharCode(96 + rIndex + 1).toUpperCase()
    });
    if (showLabel || !_this28.rowsEnumNoGaps) {
      nextRowValue = _this28.rowLabelType === _class2.LABEL_TYPES.Number ? parseInt(nextRowValue, 10) + 1 : String.fromCharCode(nextRowValue.charCodeAt(0) + 1);
    }
  });
}
function _buildSeats2() {
  ...
  rows.forEach(function (row, rIndex) {
    cols.forEach(function (col, cIndex) {
      var overlaps = _classPrivateMethodGet(_this29, _overlapsWithFixedElement, _overlapsWithFixedElement2).call(_this29, [].concat(_toConsumableArray(_this29.facilities), _toConsumableArray(_this29.corridor)), rIndex + 1, cIndex + 1);
      ...
      if (!hideSeat) {
        ...
        var allowKeyNav = _this29.allowKeyNavStatusList.includes(customSeat ? customSeat.status : "available") && (!overlaps.length || overlapsItem) ? true : false;
        var rowLabel = _classPrivateMethodGet(_this29, _getSeatRowLabel, _getSeatRowLabel2).call(_this29, rowNumber);
...
function _enumerateSeats2(section) {
  ...
  rows.forEach(function (row, rIndex) {
    _this30.seats.filter(function (s) {
      ...
      return _this30.enumDir === _class2.ENUMERATION_DIRECTION.Left ? a.col - b.col : b.col - a.col;
    }).forEach(function (s, index) {
      if (_this30.enumType === _class2.ENUMERATION_TYPES.PerRow) {
        if (rowNumber !== s.row) {
      ...
      s.index = seatIndex;
      seatLabel = _this30.seatLabelType === _class2.LABEL_TYPES.Number ? parseInt(seatLabel, 10) + 1 : String.fromCharCode(seatLabel.charCodeAt(0) + 1);
      seatIndex++;
    });
  });
}
