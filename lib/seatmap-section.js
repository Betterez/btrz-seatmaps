...
var SeatmapSection = /*#__PURE__*/function () {
  function SeatmapSection(containerId, _section) {
    var events = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];
    var classes = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : SeatmapSection.CLASSES;
    ...
    this.availableCols = _section.availableCols || 5;
    this.availableRows = _section.availableRows || 15;
    this.seatsPerRowLeft = _section.seatsPerRowLeft;
    this.numberOfSeatsPerRowLeft = 2;
    if (typeof _section.seatsPerRowLeft !== "undefined") {
      this.numberOfSeatsPerRowLeft = typeof _section.seatsPerRowLeft.key !== "undefined" ? parseInt(_section.seatsPerRowLeft.key, 10) : parseInt(_section.seatsPerRowLeft, 10);
    }
    this.seatsPerRowRight = _section.seatsPerRowRight;
    this.numberOfSeatsPerRowRight = 2;
    if (typeof _section.seatsPerRowRight !== "undefined") {
      this.numberOfSeatsPerRowRight = typeof _section.seatsPerRowRight.key !== "undefined" ? parseInt(_section.seatsPerRowRight.key, 10) : parseInt(_section.seatsPerRowRight, 10);
    }
    ...
    this.enumType = _section.enumType;
    this.enumerationType = _section.enumType && _section.enumType.key ? _section.enumType.key : _section.enumType || SeatmapSection.ENUMERATION_TYPES.Sequencial;
    this.enumDir = _section.enumDir;
    this.enumerationDir = _section.enumDir && _section.enumDir.key ? _section.enumDir.key : _section.enumDir || SeatmapSection.ENUMERATION_TYPES.Left, this.startingSeatLabel = _section.startingSeatLabel || 1, this.rowLabelType = _section.rowLabelType;
    this.rowLabelTypeKey = _section.rowLabelType && _section.rowLabelType.key ? _section.rowLabelType.key : _section.rowLabelType || SeatmapSection.LABEL_TYPES.Number;
    this.seatLabelType = _section.seatLabelType;
    this.seatLabelTypeKey = _section.seatLabelType && _section.seatLabelType.key ? _section.seatLabelType.key : _section.seatLabelType || SeatmapSection.LABEL_TYPES.Number;
    ...
    this.sectionName = _section.sectionName || "Main section";
    ...
    value: function draw() {
      ...
      if (container) {
        ...
        [].concat(_toConsumableArray(this.corridor), _toConsumableArray(this.elements), _toConsumableArray(this.seats)).forEach(function (elem) {
          ...
          e.style["grid-area"] = "".concat(elem.row, "/").concat(elem.col, "/").concat(elem.row + elem.height, "/").concat(elem.col + elem.width);
          ...
          if (elem.type === "seat") {
            e.title = _classPrivateMethodGet(_this, _getSeatTitle, _getSeatTitle2).call(_this, elem, _this.sectionName);
          }
          _this.events.forEach(function (evt) {
            if (!evt.elementType.startsWith("!") && evt.elementType === elem.type || evt.elementType.startsWith("!") && evt.elementType.slice(1) !== elem.type) {
...
function _buildCorridor2() {
  ...
  rows.forEach(function (row, rIndex) {
    ...
    _this3.corridor.push({
      ...
      col: _this3.numberOfSeatsPerRowLeft + 1,
      height: 1,
      width: 1,
      type: "corridor",
      classes: _class.CLASSES.corridor,
      label: showLabel ? nextRowValue : undefined,
      alternativeLabel: _this3.rowLabelTypeKey === _class.LABEL_TYPES.Number ? rIndex + 1 : String.fromCharCode(96 + rIndex + 1).toUpperCase()
    });
    if (showLabel || !_this3.rowsEnumNoGaps) {
      nextRowValue = _this3.rowLabelTypeKey === _class.LABEL_TYPES.Number ? parseInt(nextRowValue, 10) + 1 : String.fromCharCode(nextRowValue.charCodeAt(0) + 1);
    }
  });
}
...
function _enumerateSeats2(section) {
  ...
  rows.forEach(function (row, rIndex) {
    _this5.seats.filter(function (s) {
      ...
      return _this5.enumerationDir === _class.ENUMERATION_DIRECTION.Left ? a.col - b.col : b.col - a.col;
    }).forEach(function (s, index) {
      if (_this5.enumerationType === _class.ENUMERATION_TYPES.PerRow) {
        if (rowNumber !== s.row) {
      ...
      s.index = seatIndex;
      seatLabel = _this5.seatLabelTypeKey === _class.LABEL_TYPES.Number ? parseInt(seatLabel, 10) + 1 : String.fromCharCode(seatLabel.charCodeAt(0) + 1);
      seatIndex++;
    });
  });
}
...
function _getSeatRowLabel2(rowNumber) {
  ...
  if (seatRow) {
    ...
    seatRowLabel = seatRow.label || seatRow.alternativeLabel;
    seatRowLabel = !this.showRowLabels || this.rowLabelTypeKey === _class.LABEL_TYPES.Number ? parseInt(seatRowLabel, 10) + 1 : String.fromCharCode(seatRowLabel.charCodeAt(0) + 1);
  }
  return seatRowLabel;
}
function _getSeatTitle2(elem, sectionName) {
  var seat = elem.label ? "Seat: ".concat(elem.label) : "";
  var row = elem.rowLabel ? "Row: ".concat(elem.rowLabel) : "";
  var status = "Status: ".concat(elem.status.charAt(0).toUpperCase()).concat(elem.status.slice(1));
  var section = "Section: ".concat(sectionName);
  return "".concat(section, " - ").concat(row, " - ").concat(seat, " - ").concat(status);
}
...
function _onJumpToNextSeatNextRow2() {
  ...
  if (focus) {
    var index = parseInt(focus.dataset.index, 10);
    for (var i = index + this.numberOfSeatsPerRowLeft + this.numberOfSeatsPerRowRight; i < this.seats.length + 1; i += this.numberOfSeatsPerRowLeft + this.numberOfSeatsPerRowRight) {
      if (_classPrivateMethodGet(this, _setFocusOnSeat, _setFocusOnSeat2).call(this, i)) {
        break;
      }
    }
...
function _onJumpToPrevSeatPrevRow2() {
  ...
  if (focus) {
    var index = parseInt(focus.dataset.index, 10);
    var exception = this.lastRowNoGap && index > this.seats.length - this.numberOfSeatsPerRowRight;

    // eslint-disable-next-line max-len
    for (var i = index - this.numberOfSeatsPerRowLeft - this.numberOfSeatsPerRowRight - (exception ? 1 : 0); i > 0; i -= this.numberOfSeatsPerRowLeft + this.numberOfSeatsPerRowRight) {
      if (_classPrivateMethodGet(this, _setFocusOnSeat, _setFocusOnSeat2).call(this, i)) {
        break;
      }
    }
...
function _manageKeyboardNavigation2(evt) {
  ...
  // Arrow right
  if (evt.keyCode === 39) {
    if (this.enumerationDir === _class.ENUMERATION_DIRECTION.Left) {
