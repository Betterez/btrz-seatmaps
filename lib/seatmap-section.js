...
class SeatmapSocket {
  ...
  static listen(settings) {
    ...
    if (SeatmapSocket.channels.has(settings.idForLiveSeatmap)) {
      ...
      const newChannel = SeatmapSocket.socket.channel(`seatmap:${settings.idForLiveSeatmap}`, {
    ...
    function joinChannel(vChannel) {
      vChannel.join().receive("ok", () => {
        ...
        vChannel.on("sync:seats", payload => {
          ...
          SeatmapSocket.settings.callbacks.seatExpired(payload.expired.map(data => {
            return {
...
class SeatmapSection {
  ...
  constructor(containerId = "", section = {}, settings = {}, isBackOffice = false) {
  ...
  draw() {
    ...
    if (container) {
      container.onkeydown = evt => {
        this.#manageKeyboardNavigation(evt);
      };
      ...
      [...this.corridor, ...this.facilities, ...this.seats].forEach(elem => {
        ...
        this.events.forEach(evt => {
          ...
          if (evt.elementStatus) {
            appliesEvent = evt.elementType ? appliesEvent && evt.elementStatus.map(s => s.trim()).includes(elem.status) : appliesEvent || evt.elementStatus.map(s => s.trim()).includes(elem.status);
          }
          if (appliesEvent) {
  ...
  focusOnNextSelected(seat) {
    const seatSelected = seat ? document.querySelector(`[data-keynav='true'][data-selected=true][data-index='${seat.index}']`) : document.querySelector(`[data-keynav='true'][data-selected=true]`) || document.querySelector(`[data-keynav='true'][data-suggested=true]`);
    ...
    const statusSelector = this.isBackOffice ? ":is([data-status='available'],[data-status='reserved'])" : "[data-status='available']";
  ...
  getCapacity(statusToExclude = []) {
    return (this.seats || []).filter(s => !s.overlapped && s.status && s.type === "seat" && !statusToExclude.includes(s.status)).length;
  }
  selectElement(elem) {
  ...
  #buildCorridor() {
    ...
    rows.forEach((row, rIndex) => {
      ...
      if (showLabel || !this.rowsEnumNoGaps) {
        nextRowValue = this.rowLabelType === SeatmapSection.LABEL_TYPES.Number ? parseInt(nextRowValue, 10) + 1 : String.fromCharCode(nextRowValue.charCodeAt(0) + 1);
      }
    });
  }
  #buildSeats() {
    ...
    rows.forEach((row, rIndex) => {
      cols.forEach((col, cIndex) => {
        ...
        if (!hideSeat) {
          ...
          const allowKeyNav = (this.allowKeyNavStatusList.includes(status) || status === "reserved") && !isOverlapped;
          const seat = {
            sectionId: this.sectionId,
    ...
    let seatIndex = 1;
    rows.forEach((row, rIndex) => {
      this.seats.filter(s => s.row === rIndex + 1).sort((a, b) => this.enumDir === SeatmapSection.ENUMERATION_DIRECTION.Left ? a.col - b.col : b.col - a.col).forEach((s, index) => {
        ...
        s.index = seatIndex;
        seatLabel = this.seatLabelType === SeatmapSection.LABEL_TYPES.Number ? parseInt(seatLabel, 10) + 1 : String.fromCharCode(seatLabel.charCodeAt(0) + 1);
        seatIndex++;
      });
    });
  }
  ...
  #getElementClasses(elem) {
    ...
    if (elem.type === "driver") {
      return elem.alignment.key === SeatmapSection.FACILITY_ALIGNMENT.Left ? this.classes.driverLeft : this.classes.driverRight;
    }
    if (elem.type === "door") {
      return elem.alignment.key === SeatmapSection.FACILITY_ALIGNMENT.Left ? this.classes.doorLeft : this.classes.doorRight;
    }
    if (elem.type === "stairway") {
      return elem.orientation.key === SeatmapSection.FACILITY_ORIENTATION.Horizontal ? this.classes.stairwayHorizontal : this.classes.stairwayVertical;
    }
    if (elem.type === "accordion") {
      return elem.position.key === SeatmapSection.FACILITY_POSITION.Bottom ? this.classes.accordionBottom : this.classes.accordionTop;
    }
    return elem.classes || "";
  }
  #getSeatDataset(elem) {
  ...
  #getSeatRowLabel(rowNumber) {
    ...
    if (seatRow) {
      ...
      seatRowLabel = seatRow.label || seatRow.alternativeLabel;
      seatRowLabel = !this.showRowLabels || this.rowLabelType === SeatmapSection.LABEL_TYPES.Number ? parseInt(seatRowLabel, 10) + 1 : String.fromCharCode(seatRowLabel.charCodeAt(0) + 1);
    }
    return seatRowLabel;
  }
  ...
  #onJumpToNextSeatNextRow() {
    ...
    if (focus) {
      const index = parseInt(focus.dataset.index, 10);
      const section = focus.dataset.section;
      for (let i = index + this.seatsPerRowLeft + this.seatsPerRowRight; i < this.seats.length + 1; i += this.seatsPerRowLeft + this.seatsPerRowRight) {
        if (this.#setFocusOnSeat(i, `[data-section='${section}']`)) {
          break;
        }
      }
  ...
  #onJumpToPrevSeatPrevRow() {
    ...
    if (focus) {
      ...
      for (let i = index - this.seatsPerRowLeft - this.seatsPerRowRight - (exception ? 1 : 0); i > 0; i -= this.seatsPerRowLeft + this.seatsPerRowRight) {
        if (this.#setFocusOnSeat(i, `[data-section='${section}']`)) {
          break;
        }
      }
...
class SeatmapIframe {
  ...
  drawSeatmapIframe() {
    ...
    this.seatmap.sections.forEach((section, index) => {
      ...
      const seatmapSection = new SeatmapSection(sectionContainer.id, section, {
        fees: this.seatfees,
