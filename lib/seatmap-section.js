...
function _updateFacilities() {
  ...
  this.facilities.forEach(function (ele) {
    if (ele.type === "driver") {
      // eslint-disable-next-line no-param-reassign
      ele.width = _this26.availableCols;
      filteredElements.push(ele);
    } else if (ele.type === "accordion") {
      ele.width = _this26.availableCols;
      ele.row = ele.position.key === _SeatmapSection.FACILITY_POSITION.Top ? 1 : _this26.availableRows;
      filteredElements.push(ele);
    } else if (ele.type === "door") {
      ele.col = ele.alignment.key === _SeatmapSection.FACILITY_ALIGNMENT.Left ? 1 : _this26.availableCols;
      filteredElements.push(ele);
    } else if (_this26.availableCols + 1 >= ele.col + ele.width) {
      filteredElements.push(ele);
    }
  });
  // eslint-disable-next-line no-param-reassign
...
function _buildCorridor() {
  ...
  rows.forEach(function (row, rIndex) {
    var showLabel = _this27.showRowLabels && (!rowsWithoutLabels.length || !rowsWithoutLabels.includes(rIndex + 1));
    _this27.corridor.push({
      row: rIndex + startingRowPosition,
      col: _this27.seatsPerRowLeft + 1,
      height: 1,
      width: 1,
      type: "corridor",
      classes: _this27.classes.corridor,
      label: showLabel ? nextRowValue : undefined,
      alternativeLabel: _this27.rowLabelType === _SeatmapSection.LABEL_TYPES.Number ? rIndex + 1 : String.fromCharCode(96 + rIndex + 1).toUpperCase()
    });
    if (showLabel || !_this27.rowsEnumNoGaps) {
      nextRowValue = _this27.rowLabelType === _SeatmapSection.LABEL_TYPES.Number ? parseInt(nextRowValue, 10) + 1 : String.fromCharCode(nextRowValue.charCodeAt(0) + 1);
    }
  });
}
function _buildSeats() {
  ...
  rows.forEach(function (row, rIndex) {
    cols.forEach(function (col, cIndex) {
      var overlaps = _assertClassBrand(_SeatmapSection_brand, _this28, _overlapsWithFixedElement).call(_this28, [].concat(_toConsumableArray(_this28.facilities), _toConsumableArray(_this28.corridor)), rIndex + 1, cIndex + 1);
      ...
      if (!hideSeat) {
        ...
        var customSeat = _this28.customSeats.find(function (st) {
          return st.col === colNumber && st.row === rowNumber;
        });
        var rowLabel = _assertClassBrand(_SeatmapSection_brand, _this28, _getSeatRowLabel).call(_this28, rowNumber);
        var label = customSeat ? customSeat.label : "";
        var status = customSeat ? customSeat.status : "available";
        if (status === "reserved" && !_this28.isEditing) {
          status = _this28.isBackOffice ? "reserved" : "blocked";
        }
        var allowKeyNav = (_this28.allowKeyNavStatusList.includes(status) || status === "reserved") && !isOverlapped;
        var seat = {
          sectionId: _this28.sectionId,
          containerId: _this28.containerId,
          type: "seat",
          classes: _this28.classes.seat,
          row: rowNumber,
          col: colNumber,
          height: 1,
          width: 1,
        ...
        seat.seatId = _this28.getSeatId(seat);
        if (_this28.allowSeatOrientation) {
...
function _enumerateSeats(section) {
  ...
  var seatIndex = 1;
  rows.forEach(function (row, rIndex) {
    _this29.seats.filter(function (s) {
      return s.row === rIndex + 1;
    }).sort(function (a, b) {
      return _this29.enumDir === _SeatmapSection.ENUMERATION_DIRECTION.Left ? a.col - b.col : b.col - a.col;
    }).forEach(function (s, index) {
      if (_this29.enumType === _SeatmapSection.ENUMERATION_TYPES.PerRow) {
        if (rowNumber !== s.row) {
          seatLabel = _this29.startingSeatLabel;
          rowNumber++;
        }
      }
      s.label = s.label || seatLabel;
      s.index = seatIndex;
      seatLabel = _this29.seatLabelType === _SeatmapSection.LABEL_TYPES.Number ? parseInt(seatLabel, 10) + 1 : String.fromCharCode(seatLabel.charCodeAt(0) + 1);
      seatIndex++;
    });
  });
}
...
function _manageKeyboardNavigation(evt) {
  ...
  if (evt.keyCode >= 48 && evt.keyCode <= 57 || evt.keyCode >= 96 && evt.keyCode <= 105) {
    if (this.numPad === "") {
      setTimeout(function () {
        _assertClassBrand(_SeatmapSection_brand, _this30, _setFocusOnSeatByLabel).call(_this30, _this30.numPad);
        _this30.numPad = "";
        _assertClassBrand(_SeatmapSection_brand, _this30, _showNumPad).call(_this30, _this30.numPad);
      }, "500");
    }
    this.numPad += evt.key;
  ...
  return _createClass(SeatmapIframe, [{
    ...
    value: function expiredSeats() {
      ...
      seats.forEach(function (seat) {
        seat.seat_id = _this25.getSeatId(seat);
        SeatmapSection.changeSeatDataProp(seat, "selected", "false");
        SeatmapSection.changeSeatStatus(seat, "available");
      });
      try {
