...
var SeatmapEvents = /*#__PURE__*/function () {
  function SeatmapEvents(settings) {
    this.channel = {};
    ...
    var socket = new Phoenix.Socket(this.socketUrl, {
      params: {
        token: this.accessTicket
      }
    });
    socket.connect();
    this.channel = socket.channel("seatmap:" + this.idForLiveSeatmap, {
    ...
    this.channel.on("seat:selected", function (seat) {
      var selectedSeat = seat.selected_seat.seat_id;
      console.log("seat:new-design:selected => ", seat);
      SeatmapSection.changeSeatStatus({
    ...
    this.channel.on("seat:unselected", function (payload) {
      ...
      SeatmapSection.changeSeatStatus({
        row: unselectedSeat.row,
        col: unselectedSeat.col,
        height: 1,
        width: 1
      }, "available");
    ...
    this.channel.on("sync:seats", function (msg) {
      ...
      SeatmapEvents.seatExpired(msg.expired.map(function (seat) {
        return {
          seat_id: "".concat(seat.seat_id.row, "-").concat(seat.seat_id.col, "-").concat(seat.seat_id.label),
          row: seat.seat_id.row,
          col: seat.seat_id.col,
          height: 1,
          width: 1
        };
      }), {
  ...
  _createClass(SeatmapEvents, [{
    key: "pushSeatUnselected",
    value: function pushSeatUnselected(selector) {
      if (this.channel) {
        var payload = {
          seat: {
            leg_from: this.legFrom,
        ...
        SeatmapSection.changeSeatDataProp({
          row: selector.row,
          col: selector.col,
          height: 1,
          width: 1
    ...
    key: "pushSeatSelected",
    value: function pushSeatSelected(selector) {
      if (this.channel) {
        var payload = {
          seat: {
            leg_from: this.legFrom,
...
var SeatmapSection = /*#__PURE__*/function () {
  function SeatmapSection() {
    ...
    this.enumDir = section.enumDir || SeatmapSection.ENUMERATION_TYPES.Left, this.rowLabelType = section.rowLabelType || SeatmapSection.LABEL_TYPES.Number;
    this.seatLabelType = section.seatLabelType || SeatmapSection.LABEL_TYPES.Number;
    this.startingSeatLabel = section.startingSeatLabel || (this.seatLabelType === SeatmapSection.LABEL_TYPES.Number ? 1 : "A"), this.startingRowLabel = section.startingRowLabel || (this.rowLabelType === SeatmapSection.LABEL_TYPES.Number ? 1 : "A");
    ...
    try {
      if (settings.socketEvents) {
        ...
        this.events = [{
          ...
          type: "mouseover",
          cb: function cb(evt, e, elem, seatmapEvents) {
            seatmapEvents.callbacks.seatOver(elem, {
              tripId: settings.socketEvents.tripId,
              scheduleId: settings.socketEvents.scheduleId
            });
          }
        }, {
          elementType: "seat",
          ...
          type: "mouseout",
          cb: function cb(evt, e, elem, seatmapEvents) {
            seatmapEvents.callbacks.seatOut(elem, {
              tripId: settings.socketEvents.tripId,
              scheduleId: settings.socketEvents.scheduleId
            });
          }
          ...
          type: "click",
          cb: function cb(evt, e, elem, seatmapEvents) {
            if (e.dataset.status === "available" && seatmapEvents.callbacks.seatSelected(elem, {
              tripId: settings.socketEvents.tripId,
              scheduleId: settings.socketEvents.scheduleId
            })) {
              seatmapEvents.pushSeatSelected(elem);
            } else if (e.dataset.status === "blocked" && e.dataset.selected === "true") {
              seatmapEvents.callbacks.seatUnselected(elem, {
                tripId: settings.socketEvents.tripId,
                scheduleId: settings.socketEvents.scheduleId
              });
            }
  ...
  _createClass(SeatmapSection, [{
          ...
          _this25.events.forEach(function (evt) {
            ...
            if (appliesEvent) {
              e.addEventListener(evt.type || "click", function (target) {
                evt.cb(target, e, elem, _this25.seatmapEvents);
              });
            }
          });
    ...
    value: function changeSeatDataProp(elem, prop, value) {
      var selector = "[style*='grid-area: ".concat(elem.row, " / ").concat(elem.col, " / ").concat(elem.row + (elem.height || 1), " / ").concat(elem.col + (elem.width || 1), ";']");
      var element = document.querySelector(selector);
      if (element) {
    ...
    value: function changeSeatStatus(elem, status) {
      var sectionName = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "";
      var labels = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : SeatmapSection.LABELS;
      var seatClasses = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : [];
...
function _updateFacilities2() {
  ...
  this.facilities.forEach(function (ele) {
    if (ele.type === "driver") {
      ...
      ele.width = _this26.availableCols;
      ele.row = ele.position.key === _class.FACILITY_POSITION.Top ? 1 : _this26.availableRows;
      filteredElements.push(ele);
    } else if (ele.type === "door") {
      ele.col = ele.alignment.key === _class.FACILITY_ALIGNMENT.Left ? 1 : _this26.availableCols;
      filteredElements.push(ele);
...
function _buildCorridor2() {
  ...
  rows.forEach(function (row, rIndex) {
    ...
    _this27.corridor.push({
      ...
      alternativeLabel: _this27.rowLabelType === _class.LABEL_TYPES.Number ? rIndex + 1 : String.fromCharCode(96 + rIndex + 1).toUpperCase()
    });
    if (showLabel || !_this27.rowsEnumNoGaps) {
      nextRowValue = _this27.rowLabelType === _class.LABEL_TYPES.Number ? parseInt(nextRowValue, 10) + 1 : String.fromCharCode(nextRowValue.charCodeAt(0) + 1);
    }
  });
}
function _buildSeats2() {
  ...
  rows.forEach(function (row, rIndex) {
    cols.forEach(function (col, cIndex) {
      var overlaps = _classPrivateMethodGet(_this28, _overlapsWithFixedElement, _overlapsWithFixedElement2).call(_this28, [].concat(_toConsumableArray(_this28.facilities), _toConsumableArray(_this28.corridor)), rIndex + 1, cIndex + 1);
      ...
      if (!hideSeat) {
        ...
        var allowKeyNav = _this28.allowKeyNavStatusList.includes(customSeat ? customSeat.status : "available") && (!overlaps.length || overlapsItem) ? true : false;
        var rowLabel = _classPrivateMethodGet(_this28, _getSeatRowLabel, _getSeatRowLabel2).call(_this28, rowNumber);
...
function _enumerateSeats2(section) {
  ...
  rows.forEach(function (row, rIndex) {
    _this29.seats.filter(function (s) {
      ...
      return _this29.enumDir === _class.ENUMERATION_DIRECTION.Left ? a.col - b.col : b.col - a.col;
    }).forEach(function (s, index) {
      if (_this29.enumType === _class.ENUMERATION_TYPES.PerRow) {
        if (rowNumber !== s.row) {
      ...
      s.index = seatIndex;
      seatLabel = _this29.seatLabelType === _class.LABEL_TYPES.Number ? parseInt(seatLabel, 10) + 1 : String.fromCharCode(seatLabel.charCodeAt(0) + 1);
      seatIndex++;
    });
  });
}
...
function _getElementClasses2(elem) {
  ...
  if (elem.type === "driver") {
    return elem.alignment.key === _class.FACILITY_ALIGNMENT.Left ? this.classes.driverLeft : this.classes.driverRight;
  }
  if (elem.type === "door") {
    return elem.alignment.key === _class.FACILITY_ALIGNMENT.Left ? this.classes.doorLeft : this.classes.doorRight;
  }
  if (elem.type === "stairway") {
    return elem.orientation.key === _class.FACILITY_ORIENTATION.Horizontal ? this.classes.stairwayHorizontal : this.classes.stairwayVertical;
  }
  if (elem.type === "accordion") {
    return elem.position.key === _class.FACILITY_POSITION.Bottom ? this.classes.accordionBottom : this.classes.accordionTop;
  }
  return elem.classes || "";
}
...
function _setFocusOnSeat2(index) {
  var focus = document.querySelector("[data-focus]");
  var nextSeat = document.querySelector("[data-keynav='true'][data-index='".concat(index, "']"));
  if (nextSeat) {
    if (focus) {
      delete focus.dataset.focus;
    }
