...
var SeatmapEvents = /*#__PURE__*/function () {
  function SeatmapEvents(settings) {
    ...
    this.channel.on("seat:selected", function (seat) {
      ...
      var notificationMessage = "Seat selected: ".concat(selectedSeat.label, ": Row ").concat(selectedSeat.row, " Col\n                                    ").concat(selectedSeat.col);
      betterez.alerts.addAlert({
    ...
    this.channel.on("sync:seats", function (msg) {
      ...
      msg.expired.forEach(function (expired) {
        var notificationMessage = "Seat ".concat(expired.seat_id.label, " expired: Row ").concat(expired.seat_id.row, " Col ").concat(expired.seat_id.col);
        betterez.alerts.addAlert({
          text: notificationMessage,
          type: "warning",
          stay: true
        ...
        SeatmapSection.changeSeatStatus({
          row: expired.seat_id.row,
          col: expired.seat_id.col,
          height: 1,
          width: 1
        }, "available");
      });
    ...
    value: function draw() {
      ...
      if (container) {
        ...
        [].concat(_toConsumableArray(this.corridor), _toConsumableArray(this.facilities), _toConsumableArray(this.seats)).forEach(function (elem) {
          ...
          if (elem.type === "seat") {
            e.title = _classPrivateMethodGet(_this25, _getSeatTitle, _getSeatTitle2).call(_this25, elem, _this25.sectionName, _this25.labels);
          }
